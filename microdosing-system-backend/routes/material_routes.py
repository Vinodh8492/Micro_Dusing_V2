...
def add_material():
    try:
        ...
        # Create Material with calculated margin
        new_material = Material(
            ...
            margin=margin  # Set calculated margin
        )
...
def get_active_material():
    try:
        active_material = Material.query.filter_by(status="active").first()

        if not active_material:
            logging.warning("No active material found.")
            return jsonify({"message": "No active material found."}), 404
        ...
        logging.error(f"Database error while fetching active material: {str(e)}")
        return jsonify({"error": "Internal server error. Could not fetch active material."}), 500
